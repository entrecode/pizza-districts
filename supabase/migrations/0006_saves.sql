...
--
-- Per ADR-0003 §4.2 + §6.3 + PIZ-33 acceptance: every save mutation
-- (rng_seed at create, current_tick + engine_version at advance)
-- flows through service-role RPCs (onboarding Edge Function and
-- sim_commit_ticks). Authenticated callers have SELECT only; there
-- is no client-direct write path to this table in v0. INSERT, UPDATE
-- and DELETE are intentionally not granted to authenticated.
...
-- Authenticated callers can SELECT only their own saves.
