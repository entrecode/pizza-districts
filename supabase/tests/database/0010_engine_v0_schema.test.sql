...
-- A cannot UPDATE saves per ADR-0003 §6.3 + PIZ-33 acceptance: every
-- mutation flows through service-role RPCs (sim_commit_ticks for the
-- advance loop; onboarding Edge Function for create). Even on the
-- caller's own row.
select throws_ok(
  $$update public.saves
  ...
  '42501',
  null,
  'auth user A cannot UPDATE their own save (no UPDATE grant)'
);
...
select throws_ok(
  ...
  '42501',
  null,
  'auth user A cannot UPDATE saves.rng_seed'
);

select throws_ok(
  $$update public.saves
      set current_tick = 999
    where id = '33333333-3333-3333-3333-333333333333'$$,
  '42501',
  null,
  'auth user A cannot UPDATE saves.current_tick'
);
